...
const bitwardenGeneratedDescription = 'Generated by Bitwarden.'

function normalizeDescription(description?: string): string {
  if (description === bitwardenGeneratedDescription) {
    return `${description} #BitWarden`
  }
...
function getBindingLabels(description?: string): Array<{ key: string; label: string }> {
  ...
  for (const match of normalizeDescription(description).matchAll(labelPattern)) {
    const label = match[2]
    if (!label) continue
